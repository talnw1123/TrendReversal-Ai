...
def calculate_features(df):
    ...
    # ========================================================================
    # ADVANCED FEATURES (Phase 2 enhancement)
    # ========================================================================
    try:
        # 1. Order Flow Proxy: volume × price-change direction (signed volume)
        if 'Volume' in df.columns and 'Close' in df.columns:
            ret = df['Close'].pct_change().fillna(0)
            df['signed_volume'] = df['Volume'] * np.sign(ret)
            df['signed_volume_ma_5'] = df['signed_volume'].rolling(5).mean()
            df['signed_volume_ma_20'] = df['signed_volume'].rolling(20).mean()
            # Volume Z-score (volume relative to recent average)
            v_mean_20 = df['Volume'].rolling(20).mean()
            v_std_20 = df['Volume'].rolling(20).std() + 1e-9
            df['volume_zscore_20'] = (df['Volume'] - v_mean_20) / v_std_20

        # 2. Volatility Regime Indicators
        if 'Close' in df.columns:
            log_ret = np.log(df['Close'] / df['Close'].shift(1))
            # ATR-style volatility percentile
            vol_20 = log_ret.rolling(20).std() * np.sqrt(252)
            vol_60 = log_ret.rolling(60).std() * np.sqrt(252)
            df['vol_20d_ann'] = vol_20
            df['vol_60d_ann'] = vol_60
            # Volatility ratio: short vs long term (>1 = vol expanding)
            df['vol_ratio_20_60'] = vol_20 / (vol_60 + 1e-9)
            # Volatility regime percentile (rolling 252 days)
            df['vol_percentile_252'] = vol_20.rolling(252, min_periods=20).rank(pct=True)
        ...
        if isinstance(df.index, pd.DatetimeIndex):
            ...
            df['month_of_year'] = df.index.month.astype(float)
            # Cyclical encoding for periodicity
            df['day_sin'] = np.sin(2 * np.pi * df['day_of_week'] / 5.0)
            df['day_cos'] = np.cos(2 * np.pi * df['day_of_week'] / 5.0)
            df['month_sin'] = np.sin(2 * np.pi * df['month_of_year'] / 12.0)
            df['month_cos'] = np.cos(2 * np.pi * df['month_of_year'] / 12.0)
            # Quarter end / Year end markers (monthly OpEx proxy)
            df['is_month_end'] = (df.index.is_month_end).astype(float)
            df['is_quarter_end'] = (df.index.is_quarter_end).astype(float)

        # 4. Bid-Ask Spread Proxy (using High-Low range)
        if all(c in df.columns for c in ['High', 'Low', 'Close']):
            df['hl_spread'] = (df['High'] - df['Low']) / (df['Close'] + 1e-9)
            df['hl_spread_ma_20'] = df['hl_spread'].rolling(20).mean()
            df['hl_spread_zscore'] = (df['hl_spread'] - df['hl_spread'].rolling(60).mean()) / \
                                    (df['hl_spread'].rolling(60).std() + 1e-9)
            # Open-Close to Range ratio (intraday momentum direction)
            df['oc_to_range'] = (df['Close'] - df['Open']) / (df['High'] - df['Low'] + 1e-9)

        # 5. Momentum Persistence Features
        if 'Close' in df.columns:
            ret = df['Close'].pct_change().fillna(0)
            # Streak (consecutive up/down days)
            sign = np.sign(ret)
            df['return_sign'] = sign
            # Cumulative streak length
            streak = sign.groupby((sign != sign.shift()).cumsum()).cumcount() + 1
            df['streak_length'] = streak * sign  # signed: positive=up streak
            # Skewness and kurtosis of recent returns
            df['ret_skew_20'] = ret.rolling(20).skew()
            df['ret_kurt_20'] = ret.rolling(20).kurt()

        # 6. Price-to-MA Distance (normalized)
        if 'Close' in df.columns:
            for w in [50, 100, 200]:
                ma = df['Close'].rolling(w).mean()
                df[f'price_dist_ma{w}'] = (df['Close'] - ma) / (ma + 1e-9)
                # MA slope (trend strength)
                df[f'ma{w}_slope_20'] = (ma - ma.shift(20)) / (ma.shift(20) + 1e-9)
    except Exception as e:
