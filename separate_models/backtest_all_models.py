...
def load_model_and_objects(model_dir, model_name):
    ...
    Search order for model_dir:
      1. Given path (e.g. 'all_trad/separate_models/model_BTC_uptrend')
      2. Project root (e.g. './model_BTC_uptrend')
      3. Parent of given path (legacy fallback)
    """
    model = None
    scaler = None
    le = None
    ...
    # Build candidate directories to try (in priority order)
    candidates = [model_dir]
    # If model_dir is like 'all_trad/separate_models/model_X_Y', also try './model_X_Y'
    folder_name = os.path.basename(model_dir.rstrip('/'))
    if folder_name and folder_name not in candidates:
        candidates.append(folder_name)
        candidates.append(os.path.join('.', folder_name))

    chosen_dir = None
    for cand in candidates:
        p_keras = os.path.join(cand, f"{model_name}.keras")
        p_pkl = os.path.join(cand, f"{model_name}.pkl")
    ...
    if chosen_dir is None:
        return None, None, None, model_type

    # Load Objects from the same directory we found the model in
    scaler_path = os.path.join(chosen_dir, "scaler.pkl")
    le_path = os.path.join(chosen_dir, "label_encoder.pkl")
...
def run_simulation(signals, prices, mode='standard', trailing_stop_pct=0.0, return_positions=False):
    """Run trading simulation.

    Modes:
      'standard'    : signal 1 -> Long, signal 0 -> Short, signal 99 -> Cash (original)
      'long_bias'   : signal 1 -> Long, signal 0 -> Cash, signal 99 -> Long (default to long, no shorts)
      'long_only'   : signal 1 -> Long, else Cash (no shorts at all)
      'long_neutral': signal 1 -> Long, signal 0 -> Cash, signal 99 -> Cash (no shorts, neutral = cash)
      'bh_anchor'   : Always Long (matches B&H baseline; useful as a fallback)

    trailing_stop_pct: if > 0, exit long position when drawdown from peak exceeds this fraction
                      (re-enters when next non-bearish signal appears).
    """
    cash = INITIAL_CAPITAL
    position = 0
    equity_curve = []
    positions_log = []  # Track position at each step for trade event extraction
    # Trailing stop bookkeeping (for long positions only)
    peak_equity = INITIAL_CAPITAL
    stop_triggered = False  # When trailing stop fires, we stay flat until next non-bearish signal
    ...
    for i in range(len(signals) - 1):
        ...
        # Decide raw target position from signal+mode
        if mode == 'standard':
            if signal == 1:    raw_target = 1
            elif signal == 0:  raw_target = -1
            elif signal == 99: raw_target = 0
            else:              raw_target = 0
        elif mode == 'long_bias':
            if signal == 1:    raw_target = 1
            elif signal == 0:  raw_target = 0
            elif signal == 99: raw_target = 1
            else:              raw_target = 1
        elif mode == 'long_only':
            raw_target = 1 if signal == 1 else 0
        elif mode == 'long_neutral':
            raw_target = 1 if signal == 1 else 0
        elif mode == 'bh_anchor':
            raw_target = 1
        else:
        ...
        # Trailing stop logic: if we hit drawdown threshold while long, exit and stay out
        if trailing_stop_pct > 0 and position == 1:
            if cash < peak_equity * (1 - trailing_stop_pct):
                stop_triggered = True
        if stop_triggered:
            # Stay flat until model gives a NON-bearish signal (1 or 99)
            if signal == 0:
                target_pos = 0
            else:
                stop_triggered = False
                peak_equity = cash  # reset peak
                target_pos = raw_target
        else:
            target_pos = raw_target

        # Execute trade with cost
        ...
        # Update peak (only when long, for trailing stop reference)
        if position == 1 and cash > peak_equity:
            peak_equity = cash
    ...
    if return_positions:
        return equity_curve, positions_log
    return equity_curve


def run_simulation_weighted(bull_probs, prices, exit_thresh=0.40, entry_thresh=0.50,
                            sizing='confidence', trailing_stop_pct=0.0,
                            min_long_floor=0.0, target_vol=0.0, vol_window=20,
                            return_positions=False):
    """
    Weighted/asymmetric long-bias simulation.

    Position sizing approaches (sizing parameter):
      'confidence' : pos_size = max(0, min(1, (bull_prob - 0.5) * 2))
      'binary'     : pos_size = 1 if bull_prob >= entry_thresh else 0
      'asymmetric' : enter long when bull_prob >= entry_thresh,
                     exit to cash only when bull_prob <= exit_thresh,
                     otherwise hold previous position
                     (default to long if no prior position)
      'pyramid'    : start at floor, ramp up by 0.2 each consecutive bull signal up to 1.0,
                     reset to floor on bear signal

    min_long_floor : minimum long position to maintain (e.g., 0.3 means always >=30% long)
    trailing_stop_pct: drawdown fraction to trigger flat-out
    target_vol     : if > 0, scale position by min(1, target_vol / realized_vol_annualized)
                     reduces DD in volatile assets like BTC. Try 0.15-0.30.
    vol_window     : rolling window (days) for realized vol computation
    """
    cash = INITIAL_CAPITAL
    position = 0.0
    equity_curve = []
    ...
    peak_equity = INITIAL_CAPITAL
    stop_triggered = False
    consec_bull = 0  # for pyramid mode

    # Pre-compute rolling realized volatility for vol-targeting
    if target_vol > 0:
        log_returns = np.zeros(len(prices))
        log_returns[1:] = np.log(prices[1:] / np.maximum(prices[:-1], 1e-9))
        realized_vol = np.zeros(len(prices))
        for i in range(len(prices)):
            start = max(0, i - vol_window + 1)
            window = log_returns[start:i+1]
            if len(window) > 1:
                realized_vol[i] = np.std(window) * np.sqrt(252)
            else:
                realized_vol[i] = target_vol
    else:
        realized_vol = None

    for i in range(len(bull_probs) - 1):
        bp = bull_probs[i]
        price = prices[i]
        next_price = prices[i+1]

        # Compute target position size based on sizing strategy
        if sizing == 'confidence':
            target_pos = max(0.0, min(1.0, (bp - 0.5) * 2.0))
        elif sizing == 'binary':
            target_pos = 1.0 if bp >= entry_thresh else 0.0
        elif sizing == 'asymmetric':
            if bp >= entry_thresh:
                target_pos = 1.0
            elif bp <= exit_thresh:
                target_pos = 0.0
            else:
                target_pos = position if position > 0 else 1.0  # hold or default long
        elif sizing == 'pyramid':
            if bp >= entry_thresh:
                consec_bull = min(5, consec_bull + 1)
            elif bp <= exit_thresh:
                consec_bull = 0
            target_pos = max(min_long_floor, consec_bull * 0.2)
        else:
            target_pos = 1.0 if bp >= entry_thresh else 0.0

        # Apply long floor
        if min_long_floor > 0:
            target_pos = max(target_pos, min_long_floor)

        # Volatility targeting: scale position so portfolio vol ~= target_vol
        if target_vol > 0 and realized_vol is not None and realized_vol[i] > 1e-6:
            vol_scale = min(1.0, target_vol / realized_vol[i])
        ...
        # Execute
        if abs(position - target_pos) > 1e-6:
            cost = cash * abs(target_pos - position) * TRANSACTION_COST
            cash -= cost
            position = target_pos

        # Apply price change scaled by position (long-only here)
        if position > 0:
            cash *= (1 + position * (next_price - price) / price)

        if position > 0 and cash > peak_equity:
            peak_equity = cash
        ...
        for trend in TRENDS:
            ...
            try:
                ...
                if probs is not None:
                    # Optimize over multiple strategies + threshold + trailing stop + smoothing + vol target
                    best_thresh = 0.50
                    best_mode = 'long_bias'
                    best_trail = 0.0
                    best_score = -99999
                    ...
                    bull_probs_raw = probs[:, 1] if probs.shape[1] > 1 else probs[:, 0]

                    # ==========================================================
                    # NEW: Compute Regime Probability for gated trading
                    # ==========================================================
                    # Use ensemble regime probability to scale signals in confused markets
                    try:
                        regime_full = RegimeDetector.detect_ensemble_proba(df, market_name=market)
                        # Align regime probs to bull_probs (skip lookback rows)
                        regime_aligned = regime_full[-len(bull_probs_raw):] \
                            if len(regime_full) >= len(bull_probs_raw) \
                            else np.full(len(bull_probs_raw), 0.5)
                        regime_aligned = np.nan_to_num(regime_aligned, nan=0.5)
                        # For "uptrend" model, multiply by regime_prob (high in uptrend regime)
                        # For "downtrend" model, multiply by (1 - regime_prob) (high in downtrend)
                        if trend == 'uptrend':
                            regime_gate = regime_aligned
                        else:
                            regime_gate = 1.0 - regime_aligned
                        # Smooth regime gate
                        regime_gate = pd.Series(regime_gate).ewm(span=10, adjust=False).mean().values
                        regime_gate = np.clip(regime_gate, 0.0, 1.0)
                    except Exception as _e:
                        regime_gate = np.full(len(bull_probs_raw), 1.0)

                    # Generate smoothed versions of bull_probs (EMA) to reduce signal noise
                    def _ema(arr, span):
                        s = pd.Series(arr).ewm(span=span, adjust=False).mean().values
                        return s
                    bp_variants = {
                        'raw':    bull_probs_raw,
                        'ema3':   _ema(bull_probs_raw, 3),
                        'ema5':   _ema(bull_probs_raw, 5),
                        'ema10':  _ema(bull_probs_raw, 10),
                    }
                    # Regime-gated variant: blend bull_prob with regime confidence
                    # When regime is confused (~0.5), pull bull_prob toward 0.5 (no trade)
                    # When regime is clear (>0.7), use bull_prob as-is
                    bp_regime_gated = bull_probs_raw * regime_gate + 0.5 * (1 - regime_gate)
                    bp_variants['regime_gated'] = bp_regime_gated
                    bp_variants['regime_ema5']  = _ema(bp_regime_gated, 5)

                    # Compute B&H baseline ONCE for scoring
                    bnh_baseline = (prices[-1] - prices[0]) / prices[0] * 100

                    thresholds = np.arange(0.50, 0.90, 0.02)
                    trail_options = [0.0, 0.10, 0.15, 0.20]
                    vol_targets = [0.0, 0.15, 0.20, 0.25]   # 0 = off, 15-25% annual vol
                    discrete_modes = ['long_bias', 'long_only', 'long_neutral', 'standard', 'bh_anchor']
                    weighted_strategies = [
                        # (sizing, exit_thresh, entry_thresh, min_floor)
                        ('confidence', 0.40, 0.50, 0.0),
                        ('confidence', 0.40, 0.50, 0.3),
                        ('confidence', 0.40, 0.50, 0.5),
                        ('asymmetric', 0.30, 0.55, 0.0),
                        ('asymmetric', 0.35, 0.55, 0.3),
                        ('asymmetric', 0.40, 0.55, 0.5),
                        ('pyramid',    0.45, 0.55, 0.2),
                    ]

                    print(f"  Optimizing strategies (modes + weighted + smoothing + vol-target) ...")
                    print(f"  Buy & Hold baseline: {bnh_baseline:+.2f}%")

                    def _evaluate(eq_curve, label):
                        nonlocal best_score, best_thresh, best_mode, best_trail, best_stats
                        if not eq_curve:
                            return
                        ...
                        if len(downside) > 1 and np.std(downside) > 1e-9:
                            sortino = np.mean(daily_returns) / np.std(downside) * np.sqrt(252)
                        else:
                            sortino = 0.0
                        peak = np.maximum.accumulate(eq_arr)
                        dd_curve = (eq_arr - peak) / peak
                        max_dd_pct = abs(np.min(dd_curve)) * 100
                        # Calmar = annual return / |max DD|
                        years = max(len(eq_arr) / 252.0, 0.5)
                        ann_ret = (final_val / INITIAL_CAPITAL) ** (1/years) - 1
                        calmar = (ann_ret * 100) / max_dd_pct if max_dd_pct > 1.0 else 0.0
                        alpha = ret - bnh_baseline

                        # Composite score:
                        #   - Alpha (most important — beat B&H)
                        #   - Sortino (downside-aware Sharpe)
                        #   - Calmar bonus (return / DD ratio)
                        #   - DD penalty (squared above 30% to crush large DDs)
                        dd_penalty = 0.10 * max_dd_pct
                        if max_dd_pct > 30:
                            dd_penalty += 0.5 * (max_dd_pct - 30)
                        score = alpha + 0.5 * sortino + 2.0 * calmar - dd_penalty
                        if score > best_score:
                            best_score = score
                            best_stats = (ret, calculate_drawdown(pd.Series(eq_curve)),
                                          final_val, eq_curve, alpha, sharpe, max_dd_pct, label)
                            print(f"    New Best: {label} | Ret {ret:+.2f}% | "
                                  f"Alpha {alpha:+.2f}% | Sortino {sortino:.2f} | "
                                  f"Calmar {calmar:.2f} | DD -{max_dd_pct:.1f}%")

                    # 1. Discrete-signal modes (NO smoothing — uses argmax classes directly)
                    for mode_try in discrete_modes:
                        for trail in trail_options:
                            if mode_try == 'bh_anchor':
                                signals_th = feature_class
                                eq = run_simulation(signals_th, prices, mode=mode_try,
                                                    trailing_stop_pct=trail)
                                lbl = f"Mode={mode_try:12s} Trail={trail:.2f}"
                                prev = best_score
                                _evaluate(eq, lbl)
                                if best_score > prev:
                                    best_thresh = 0.50; best_mode = mode_try; best_trail = trail
                                continue
                            for th in thresholds:
                                ...
                                eq = run_simulation(signals_th, prices, mode=mode_try,
                                                    trailing_stop_pct=trail)
                                lbl = f"Mode={mode_try:12s} Th={th:.2f} Trail={trail:.2f}"
                                prev = best_score
                                _evaluate(eq, lbl)
                                if best_score > prev:
                                    best_thresh = th; best_mode = mode_try; best_trail = trail

                    # 2. Weighted strategies × Smoothing × Vol-Targeting
                    for bp_name, bp_arr in bp_variants.items():
                        for sizing, ex_t, en_t, floor in weighted_strategies:
                            for trail in trail_options:
                                for vt in vol_targets:
                                    eq = run_simulation_weighted(
                                        bp_arr, prices,
                                        exit_thresh=ex_t, entry_thresh=en_t,
                                        sizing=sizing, trailing_stop_pct=trail,
                                        min_long_floor=floor, target_vol=vt,
                                    )
                                    lbl = (f"W({sizing},{bp_name}) ex={ex_t:.2f} en={en_t:.2f} "
                                           f"floor={floor:.1f} Trail={trail:.2f} VolT={vt:.2f}")
                                    prev = best_score
                                    _evaluate(eq, lbl)
                                    if best_score > prev:
                                        best_thresh = en_t
                                        best_mode = f"W({sizing},{bp_name},f{floor:.1f},vt{vt:.2f})"
                                        best_trail = trail

                    # Finalize
                    if best_stats:
                        total_return, max_dd, final_equity, equity_curve, alpha, sharpe, max_dd_pct, label = best_stats
                        # Reconstruct signals for plotting (best-effort)
                        if best_mode.startswith('weighted_'):
                            # For weighted modes we don't have discrete signals;
                            # synthesize from bull_probs for plot annotation
                            signals = np.where(bull_probs_arr >= 0.5, 1, 0)
                        else:
                            neutral_mask = max_prob < best_thresh
                            signals = np.where(neutral_mask, 99, feature_class)
                        print(f"  > SELECTED: {label}")
                        print(f"    Ret={total_return:+.2f}% | Alpha={alpha:+.2f}% | "
                              f"Sharpe={sharpe:.2f} | DD={max_dd_pct:.2f}%")
                    else:
...
def main_combined_v2():
    """
    Combined Backtest V2 — One row per market (like trading_system.py).

    Loads BOTH uptrend & downtrend models per market, uses regime detection
    (super_regime ensemble) to decide which model's probability to use at each
    time step. Then runs the FULL multi-strategy optimization (modes + weighted
    + smoothing + vol-target + regime-gating) on the combined bull_probs.

    Output: 1 row per market (5 total instead of 10).
    """
    print("=" * 70)
    print("COMBINED BACKTEST V2 — Regime-Switch + Multi-Strategy Optimization")
    print("=" * 70)
    ...
    for market in MARKETS:
        ...
        regime_method = best_regime_map.get(market, 'HMM')
        up_name = best_models_map.get((market, 'uptrend'))
        down_name = best_models_map.get((market, 'downtrend'))
        if not up_name or not down_name:
            print(f"   Missing models for {market}. Skipping.")
            continue
        print(f"   Regime: {regime_method} | ↑{up_name} ↓{down_name}")
        ...
        # 2. Detect regime via Super Regime (ensemble + smoothing + adaptive)
        try:
            regime = RegimeDetector.detect_super_regime(df, market_name=market)
            regime = np.asarray(regime, dtype=int)
        except Exception as e:
            print(f"   Regime fallback (SMA200): {e}")
            sma = df['Close'].rolling(200).mean()
            regime = (df['Close'] > sma).astype(int).fillna(0).values

        # 3. Load both models
        up_dir = os.path.join(MODEL_BASE_DIR, f"model_{market}_uptrend")
        down_dir = os.path.join(MODEL_BASE_DIR, f"model_{market}_downtrend")
        up_model, up_scaler, _, up_type = load_model_and_objects(up_dir, up_name)
        down_model, down_scaler, _, down_type = load_model_and_objects(down_dir, down_name)
        if not up_model or not down_model:
            print(f"   Model load failed."); continue

        # 4. Build feature windows
        selected_cols = get_selected_features(df)
        if not selected_cols:
            print(f"   No features."); continue
        for c in selected_cols:
            if c not in df.columns: df[c] = 0
        feat = df[selected_cols].values

        check_lb = LOOKBACK
        if up_type == 'ml' and hasattr(up_model, 'n_features_in_'):
            n_exp = up_model.n_features_in_
            if len(selected_cols) > 0:
                check_lb = n_exp // len(selected_cols)
        if len(feat) <= check_lb:
            print(f"   Lookback too long."); continue
        ...
        X_up_list, X_down_list = [], []
        for i in range(check_lb, len(feat_s)):
            w = feat_s[i - check_lb:i]
            X_up_list.append(w.flatten() if up_type == 'ml' else w)
            X_down_list.append(w.flatten() if down_type == 'ml' else w)
        X_up = np.array(X_up_list); X_down = np.array(X_down_list)

        # 5. Predictions from both models
        try:
            if up_type == 'keras':
                p_up = up_model.predict(X_up, verbose=0, batch_size=256)
            elif hasattr(up_model, 'predict_proba'):
                p_up = up_model.predict_proba(X_up)
            else:
                preds = up_model.predict(X_up)
                p_up = np.zeros((len(preds), 2))
                for i, s in enumerate(preds): p_up[i, int(s)] = 1.0

            if down_type == 'keras':
                p_dn = down_model.predict(X_down, verbose=0, batch_size=256)
            elif hasattr(down_model, 'predict_proba'):
                p_dn = down_model.predict_proba(X_down)
            else:
                preds = down_model.predict(X_down)
                p_dn = np.zeros((len(preds), 2))
                for i, s in enumerate(preds): p_dn[i, int(s)] = 1.0
        except Exception as e:
            print(f"   Predict error: {e}"); continue

        # 6. Combine: pick uptrend model's prob when regime=1, else downtrend's
        valid_idx = list(range(check_lb, len(feat)))
        regime_aligned = regime[valid_idx]
        prices = df['Close'].iloc[valid_idx].values
        bnh = (prices[-1] - prices[0]) / prices[0] * 100

        bull_probs = np.zeros(len(regime_aligned))
        max_probs = np.zeros(len(regime_aligned))
        for i in range(len(regime_aligned)):
            if regime_aligned[i] == 1:
                bp = p_up[i, 1] if p_up.shape[1] > 1 else p_up[i, 0]
                mp = float(np.max(p_up[i]))
            else:
                bp = p_dn[i, 1] if p_dn.shape[1] > 1 else p_dn[i, 0]
                mp = float(np.max(p_dn[i]))
            bull_probs[i] = bp
            max_probs[i] = mp
        feature_class = (bull_probs >= 0.5).astype(int)

        # 7. Get regime probability for gating (continuous)
        try:
            reg_full = RegimeDetector.detect_ensemble_proba(df, market_name=market)
            reg_aligned_p = reg_full[-len(bull_probs):] if len(reg_full) >= len(bull_probs) else np.full(len(bull_probs), 0.5)
            reg_aligned_p = np.nan_to_num(reg_aligned_p, nan=0.5)
            # For combined, the regime gate amplifies confidence in CURRENT regime
            # We trust the chosen model more when its regime is dominant
            regime_confidence = np.where(regime_aligned == 1, reg_aligned_p, 1.0 - reg_aligned_p)
            regime_confidence = pd.Series(regime_confidence).ewm(span=10, adjust=False).mean().values
            regime_confidence = np.clip(regime_confidence, 0.0, 1.0)
        except Exception:
            regime_confidence = np.full(len(bull_probs), 1.0)

        # 8. Multi-Strategy Optimization (same as main())
        def _ema(arr, span):
            return pd.Series(arr).ewm(span=span, adjust=False).mean().values
        bp_variants = {
            'raw': bull_probs,
            'ema3': _ema(bull_probs, 3),
            'ema5': _ema(bull_probs, 5),
            'ema10': _ema(bull_probs, 10),
            'regime_gated': bull_probs * regime_confidence + 0.5 * (1 - regime_confidence),
        }
        ...
        # Threshold range: include low values (0.40-0.50) to allow more signals through
        # Lower threshold = fewer signals filtered = more active trades
        thresholds = np.concatenate([
            np.arange(0.40, 0.51, 0.01),    # new: low thresholds (more trades)
            np.arange(0.51, 0.66, 0.01),    # fine grid
            np.arange(0.66, 0.86, 0.02),    # coarse (fewer trades, kept as fallback)
        ])
        trail_options = [0.0, 0.10, 0.15, 0.20]
        vol_targets = [0.0, 0.15, 0.20, 0.25]
        discrete_modes = ['long_bias', 'long_only', 'long_neutral', 'standard']
        weighted_strategies = [
            ('confidence', 0.40, 0.50, 0.0),
            ('confidence', 0.40, 0.50, 0.3),
            ('confidence', 0.40, 0.50, 0.5),
            ('confidence', 0.35, 0.48, 0.0),   # new: lower entry = more trades
            ('confidence', 0.35, 0.48, 0.3),
            ('asymmetric', 0.30, 0.52, 0.0),
            ('asymmetric', 0.35, 0.52, 0.3),
            ('asymmetric', 0.40, 0.52, 0.5),
            ('asymmetric', 0.30, 0.55, 0.0),
            ('asymmetric', 0.40, 0.55, 0.5),
            ('asymmetric', 0.25, 0.48, 0.0),   # new: aggressive entry/exit
            ('asymmetric', 0.25, 0.48, 0.3),
            ('pyramid',    0.45, 0.52, 0.2),
            ('pyramid',    0.45, 0.55, 0.2),
            ('pyramid',    0.35, 0.48, 0.2),   # new: lower entry pyramid
            ('binary',     0.40, 0.52, 0.0),   # active simple binary
            ('binary',     0.40, 0.52, 0.3),
            ('binary',     0.35, 0.48, 0.0),   # new: lower entry binary
            ('binary',     0.35, 0.48, 0.3),
        ]

        N_TOTAL = len(prices)  # Reference for trade-count normalization

        def _evaluate(eq, label, positions=None):
            nonlocal best_score, best_stats, best_label
            if not eq: return
            arr = np.array(eq)
            ret = (arr[-1] - INITIAL_CAPITAL) / INITIAL_CAPITAL * 100
            dr = np.diff(arr) / arr[:-1]
            sharpe = np.mean(dr) / (np.std(dr) + 1e-9) * np.sqrt(252) if len(dr) > 1 else 0
            downside = dr[dr < 0]
            sortino = np.mean(dr) / (np.std(downside) + 1e-9) * np.sqrt(252) if len(downside) > 1 else 0
            peak = np.maximum.accumulate(arr)
            dd = abs(np.min((arr - peak) / peak)) * 100
            yrs = max(len(arr) / 252.0, 0.5)
            ann = (arr[-1] / INITIAL_CAPITAL) ** (1/yrs) - 1
            calmar = (ann * 100) / dd if dd > 1.0 else 0
            alpha = ret - bnh
            dd_penalty = 0.10 * dd + (0.5 * (dd - 30) if dd > 30 else 0)

            # Win-rate bonus from daily strategy returns (rewards correct predictions)
            active_days = dr[np.abs(dr) > 1e-9]
            if len(active_days) > 10:
                win_rate = np.mean(active_days > 0) * 100
            else:
                win_rate = 50.0
            # Win-rate bonus: linear above 50%, mild penalty below
            wr_bonus = (win_rate - 50) * 0.4   # +2.0 for 55%, +4.0 for 60%

            # Trade frequency from position changes
            if positions is not None and len(positions) > 1:
                pos_arr = np.array(positions)
                trade_count = int(np.sum(np.abs(np.diff(pos_arr)) > 0.05))
            else:
                trade_count = int(np.sum(np.abs(np.diff(dr)) > 0.001))

            # Trade bonus: reward active strategies (more trades = more opportunities)
            if alpha > 0:
                # Profitable — reward higher trade counts progressively
                if trade_count >= 60:
                    trade_bonus = 5.0
                elif trade_count >= 40:
                    trade_bonus = 4.0
                    ...
                    trade_bonus = -1.0    # even profitable passivity gets penalized
            else:
                # Losing — penalize extra if also inactive
                trade_bonus = -1.0 if trade_count < 5 else 0.0

            # Inactivity penalty: any strategy with very few trades is penalized
            # regardless of alpha (we want the model to actively trade)
            if trade_count < 5:
                inactivity_penalty = 5.0   # strong: near-buy-and-hold
            elif trade_count < 10:
                inactivity_penalty = 3.0   # moderate
            elif trade_count < 20:
                inactivity_penalty = 1.0   # mild nudge
            else:
                inactivity_penalty = 0.0

            # Composite score: Alpha first, then quality (Sortino + Calmar),
            # win-rate (correct predictions), trade activity bonus when profitable
            score = (
                1.5 * alpha          # alpha is dominant — increased weight
                + 0.5 * sortino
                + 1.5 * calmar
                + wr_bonus
                + trade_bonus
            ...
            if score > best_score:
                best_score = score
                best_stats = (ret, dd, alpha, sharpe, sortino, calmar, arr)
                best_label = label

        # Discrete modes (now WITH position tracking for trade count)
        for mode_try in discrete_modes:
            for trail in trail_options:
                for th in thresholds:
                    sigs = np.where(max_probs < th, 99, feature_class)
                    eq, pos = run_simulation(sigs, prices, mode=mode_try,
                                             trailing_stop_pct=trail, return_positions=True)
                    _evaluate(eq, f"Mode={mode_try} Th={th:.2f} Trail={trail:.2f}", positions=pos)

        # bh_anchor included as fallback only (test once per trail)
        for trail in trail_options:
            eq, pos = run_simulation(feature_class, prices, mode='bh_anchor',
                                     trailing_stop_pct=trail, return_positions=True)
            _evaluate(eq, f"Mode=bh_anchor Trail={trail:.2f}", positions=pos)

        # Weighted strategies × smoothing × vol-target (with position tracking)
        for bp_name, bp_arr in bp_variants.items():
            for sizing, ex_t, en_t, floor in weighted_strategies:
                for trail in trail_options:
                    for vt in vol_targets:
                        eq, pos = run_simulation_weighted(
                            bp_arr, prices,
                            exit_thresh=ex_t, entry_thresh=en_t,
                            sizing=sizing, trailing_stop_pct=trail,
                            min_long_floor=floor, target_vol=vt,
                        ...
                        lbl = f"W({sizing},{bp_name}) ex={ex_t:.2f} en={en_t:.2f} f={floor:.1f} T={trail:.2f} VT={vt:.2f}"
                        _evaluate(eq, lbl, positions=pos)

        if not best_stats:
            print(f"   Optimization failed."); continue

        ret, dd, alpha, sharpe, sortino, calmar, eq_curve = best_stats
        final = eq_curve[-1]
        print(f"   📊 BEST: {best_label}")
        print(f"   Strategy: {ret:+.2f}%  B&H: {bnh:+.2f}%  Alpha: {alpha:+.2f}%  DD: -{dd:.2f}%")
        print(f"   Sharpe: {sharpe:.2f}  Sortino: {sortino:.2f}  Calmar: {calmar:.2f}  Final: ${final:,.2f}")
        ...
        # ──────────────────────────────────────────────────────────────────
        # PLOT: Per-market detailed chart (Price + Equity + Regime + Drawdown)
        # Re-run best strategy with return_positions=True for trade markers
        # ──────────────────────────────────────────────────────────────────
        try:
            positions_log = _rerun_best_strategy_with_positions(
                best_label, bp_variants, max_probs, feature_class, prices,
            )
            _plot_combined_v2_market(
                market=market, dates=df.index[valid_idx], prices=prices,
                regime=regime_aligned, eq_curve=np.array(eq_curve),
                positions=positions_log,
                bnh=bnh, ret=ret, alpha=alpha, dd=dd, sharpe=sharpe,
                up_name=up_name, down_name=down_name,
                strategy_label=best_label,
            )
        except Exception as e:
            print(f"   ⚠️ Plot error: {e}")

        if up_type == 'keras' or down_type == 'keras':
            tf.keras.backend.clear_session()

    # Save & print summary
    if results:
        res_df = pd.DataFrame(results)
        out_csv = "backtest_combined_v2_results.csv"
        res_df.to_csv(out_csv, index=False)
        print("\n" + "="*70)
        print("FINAL RESULTS (Combined V2 — One Row per Market)")
        print("="*70)
        print(res_df[['Market','Regime','UpModel','DownModel','BestStrategy',
                      'Return (%)','B&H (%)','Alpha (%)','Max DD (%)','Final $']].to_string(index=False))
        print(f"\n📊 Summary:")
        print(f"   Avg Alpha:    {res_df['Alpha (%)'].mean():+.2f}%")
        print(f"   Beat B&H:     {(res_df['Alpha (%)'] > 0).sum()}/{len(res_df)}")
        print(f"   Total Final:  ${res_df['Final $'].sum():,.2f} (Initial ${len(res_df)*INITIAL_CAPITAL:,.0f})")
        print(f"\n💾 Saved to {out_csv}")

        # Combined Dashboard: 4-panel summary chart
        try:
            _plot_combined_v2_dashboard(res_df)
        except Exception as e:
            print(f"⚠️ Dashboard plot error: {e}")
    else:
        print("\nNo results generated.")


def _rerun_best_strategy_with_positions(strategy_label, bp_variants, max_probs, feature_class, prices):
    """Re-run the BEST strategy with return_positions=True to get position history.

    Parses strategy_label like:
      'Mode=long_bias Th=0.74 Trail=0.00'   (discrete mode)
      'W(pyramid,raw) ex=0.45 en=0.55 f=0.2 T=0.00 VT=0.00'  (weighted)
    """
    import re

    # Discrete mode pattern
    m_discrete = re.match(
        r'Mode=(\S+)(?:\s+Th=([\d.]+))?\s+Trail=([\d.]+)', strategy_label
    )
    if m_discrete:
        mode_try = m_discrete.group(1)
        th = float(m_discrete.group(2)) if m_discrete.group(2) else 0.50
        trail = float(m_discrete.group(3))
        if mode_try == 'bh_anchor':
            sigs = feature_class
        else:
            sigs = np.where(max_probs < th, 99, feature_class)
        _, positions = run_simulation(sigs, prices, mode=mode_try,
                                      trailing_stop_pct=trail, return_positions=True)
        return positions

    # Weighted pattern: W(sizing,bp_name) ex=X en=Y f=Z T=W VT=V
    m_weighted = re.match(
        r'W\((\w+),(\w+)\)\s+ex=([\d.]+)\s+en=([\d.]+)\s+f=([\d.]+)\s+T=([\d.]+)\s+VT=([\d.]+)',
        strategy_label
    )
    if m_weighted:
        sizing = m_weighted.group(1)
        bp_name = m_weighted.group(2)
        ex_t = float(m_weighted.group(3))
        en_t = float(m_weighted.group(4))
        floor = float(m_weighted.group(5))
        trail = float(m_weighted.group(6))
        vt = float(m_weighted.group(7))
        bp_arr = bp_variants.get(bp_name, bp_variants.get('raw'))
        _, positions = run_simulation_weighted(
            bp_arr, prices,
            exit_thresh=ex_t, entry_thresh=en_t,
            sizing=sizing, trailing_stop_pct=trail,
            min_long_floor=floor, target_vol=vt,
...
def _plot_combined_v2_market(market, dates, prices, regime, eq_curve, bnh, ret, alpha,
                             dd, sharpe, up_name, down_name, strategy_label,
                             positions=None,
                             save_dir='model/backtest_plots/combined_v2'):
    """Per-market 4-panel plot: Price + Regime / Equity vs B&H / Drawdown / Trade activity."""
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    # Align: eq_curve length is len(prices)-1 (skip first day, like run_simulation)
    plot_dates = dates[1:]
    plot_prices = prices[1:]
    plot_bnh = (prices[1:] / prices[0]) * INITIAL_CAPITAL
    plot_eq = eq_curve
    ...
    # Drawdown curve
    peak = np.maximum.accumulate(plot_eq)
    dd_curve = (plot_eq - peak) / peak * 100

    fig, axes = plt.subplots(3, 1, figsize=(14, 11), sharex=True,
                             gridspec_kw={'height_ratios': [2, 2, 1]})

    # ── Panel 1: Price + Regime background + Buy/Sell markers ──
    ax1 = axes[0]
    ax1.plot(plot_dates, plot_prices, color='black', linewidth=1.2, alpha=0.85, label='Price', zorder=2)

    # Regime shading
    if regime is not None and len(regime) >= len(plot_dates):
        plot_regime = regime[1:1+len(plot_dates)] if len(regime) > len(plot_dates) else regime[:len(plot_dates)]
        if len(plot_regime) == len(plot_dates):
            start_idx = 0
            curr = plot_regime[0]
            for i in range(1, len(plot_regime)):
                if plot_regime[i] != curr:
                    color = '#A8E6A1' if curr == 1 else '#FFB3B3'
                    ax1.axvspan(plot_dates[start_idx], plot_dates[i], color=color, alpha=0.35, zorder=0)
                    start_idx = i
                    curr = plot_regime[i]
            color = '#A8E6A1' if curr == 1 else '#FFB3B3'
            ax1.axvspan(plot_dates[start_idx], plot_dates[-1], color=color, alpha=0.35, zorder=0)

    # Buy/Sell markers from position changes
    n_buy_markers = 0
    n_sell_markers = 0
    if positions is not None and len(positions) > 0:
        # positions is length N-1 (matching equity_curve)
        # plot_dates and plot_prices are also N-1
        prev_pos = 0.0
        for i, pos in enumerate(positions):
            if i >= len(plot_dates):
                break
            # Detect transitions: 0 -> long (buy), long -> 0 (sell)
            # Position can be float in weighted sims (0.0 to 1.0)
            BUY_THRESHOLD = 0.05  # Position > 5% counts as "long"
            was_long = prev_pos > BUY_THRESHOLD
            is_long = pos > BUY_THRESHOLD
            was_short = prev_pos < -BUY_THRESHOLD
            is_short = pos < -BUY_THRESHOLD

            if not was_long and is_long:
                # New long entry — Buy
                ax1.scatter(plot_dates[i], plot_prices[i],
                            marker='^', s=120, color='#2E7D32',
                            edgecolors='white', linewidths=1.2, zorder=10)
                n_buy_markers += 1
            elif was_long and not is_long:
                # Exit long — Sell
                ax1.scatter(plot_dates[i], plot_prices[i],
                            marker='v', s=120, color='#C62828',
                            edgecolors='white', linewidths=1.2, zorder=10)
                n_sell_markers += 1
            elif not was_short and is_short:
                # Short entry (only in 'standard' mode)
                ax1.scatter(plot_dates[i], plot_prices[i],
                            marker='v', s=120, color='#FF6F00',
                            edgecolors='white', linewidths=1.2, zorder=10)
                n_sell_markers += 1
            elif was_short and not is_short:
                # Cover short
                ax1.scatter(plot_dates[i], plot_prices[i],
                            marker='^', s=120, color='#1976D2',
                            edgecolors='white', linewidths=1.2, zorder=10)
                n_buy_markers += 1
            prev_pos = pos

    ax1.set_title(f'{market} — Price + Regime + Trade Signals (Up:{up_name} / Down:{down_name})',
                  fontsize=13, fontweight='bold')
    ax1.set_ylabel('Price ($)', fontsize=11)
    ax1.grid(True, alpha=0.3)

    # Combined legend for regime colors AND trade markers
    import matplotlib.patches as mpatches
    from matplotlib.lines import Line2D
    legend_items = [
        mpatches.Patch(color='#A8E6A1', alpha=0.5, label='Uptrend Regime'),
        mpatches.Patch(color='#FFB3B3', alpha=0.5, label='Downtrend Regime'),
        Line2D([0], [0], marker='^', color='w', markerfacecolor='#2E7D32',
               markersize=10, label=f'Buy ({n_buy_markers})'),
        Line2D([0], [0], marker='v', color='w', markerfacecolor='#C62828',
               markersize=10, label=f'Sell ({n_sell_markers})'),
    ]
    ax1.legend(handles=legend_items, loc='upper left', fontsize=9, ncol=2)

    # ── Panel 2: Equity Curve vs B&H ──
    ax2 = axes[1]
    ax2.plot(plot_dates, plot_bnh, color='gray', linestyle='--', linewidth=1.4,
             label=f'Buy & Hold ({bnh:+.2f}%)', alpha=0.85)
    color_strategy = '#2E7D32' if alpha > 0 else '#C62828'
    ax2.plot(plot_dates, plot_eq, color=color_strategy, linewidth=2.2,
             label=f'Strategy ({ret:+.2f}%, Alpha {alpha:+.2f}%)')
    ax2.axhline(INITIAL_CAPITAL, color='black', linewidth=0.6, alpha=0.4)
    ax2.fill_between(plot_dates, INITIAL_CAPITAL, plot_eq,
                     where=(plot_eq > INITIAL_CAPITAL), color=color_strategy, alpha=0.10)
    ax2.fill_between(plot_dates, INITIAL_CAPITAL, plot_eq,
                     where=(plot_eq <= INITIAL_CAPITAL), color='red', alpha=0.10)
    ax2.set_ylabel('Equity ($)', fontsize=11)
    ax2.set_title(f'Strategy: {strategy_label}', fontsize=11)
    ax2.legend(loc='upper left', fontsize=10)
    ax2.grid(True, alpha=0.3)

    # ── Panel 3: Drawdown ──
    ax3 = axes[2]
    ax3.fill_between(plot_dates, dd_curve, 0, color='#C62828', alpha=0.4)
    ax3.plot(plot_dates, dd_curve, color='#C62828', linewidth=1)
    ax3.set_ylabel('Drawdown (%)', fontsize=11)
    ax3.set_xlabel('Date', fontsize=11)
    ax3.set_title(f'Max DD: -{dd:.2f}%   |   Sharpe: {sharpe:.2f}', fontsize=10)
    ax3.grid(True, alpha=0.3)
    ax3.set_ylim(min(dd_curve.min() * 1.1, -1), 1)

    plt.suptitle(f'Combined V2 Backtest — {market}',
                 fontsize=15, fontweight='bold', y=1.00)
    plt.tight_layout()
    filepath = os.path.join(save_dir, f'{market}_combined_v2.png')
    plt.savefig(filepath, dpi=140, bbox_inches='tight')
    plt.close()
    print(f"   📊 Plot saved: {filepath}")


def _plot_combined_v2_dashboard(res_df, save_dir='model/backtest_plots/combined_v2'):
    """4-panel summary dashboard across all markets."""
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    import matplotlib.patches as mpatches

    markets = res_df['Market'].tolist()
    market_colors = {'BTC':'#F2A900','Gold':'#FFD700','Thai':'#0088CC','UK':'#012169','US':'#B22234'}
    colors = [market_colors.get(m, '#888') for m in markets]

    fig, axes = plt.subplots(2, 2, figsize=(15, 11))
    fig.suptitle('Combined V2 Backtest — Summary Dashboard',
                 fontsize=16, fontweight='bold', y=1.00)

    # Panel 1: Strategy vs B&H
    ax = axes[0, 0]
    x = np.arange(len(markets))
    w = 0.38
    ax.bar(x - w/2, res_df['Return (%)'], w, label='Strategy',
           color='#2E86AB', edgecolor='white')
    ax.bar(x + w/2, res_df['B&H (%)'], w, label='B&H',
           color='#E07A5F', edgecolor='white')
    for i, (s, b) in enumerate(zip(res_df['Return (%)'], res_df['B&H (%)'])):
        ax.text(i - w/2, s + 1.5, f'{s:+.1f}%', ha='center', fontsize=9, fontweight='bold')
        ax.text(i + w/2, b + 1.5, f'{b:+.1f}%', ha='center', fontsize=8.5)
    ax.set_xticks(x); ax.set_xticklabels(markets, fontweight='bold')
    ax.set_ylabel('Return (%)', fontsize=11)
    ax.set_title('Strategy Return vs Buy & Hold', fontsize=12, fontweight='bold')
    ax.axhline(0, color='black', linewidth=0.5)
    ax.legend(); ax.grid(axis='y', alpha=0.3)

    # Panel 2: Alpha
    ax = axes[0, 1]
    alpha_vals = res_df['Alpha (%)'].values
    bar_colors = ['#2E7D32' if a > 0 else '#C62828' for a in alpha_vals]
    ax.bar(markets, alpha_vals, color=bar_colors, edgecolor='white', width=0.55)
    for i, a in enumerate(alpha_vals):
        offset = 1.0 if a >= 0 else -1.5
        ax.text(i, a + offset, f'{a:+.2f}%', ha='center', fontsize=10, fontweight='bold')
    ax.axhline(0, color='black', linewidth=0.6)
    ax.set_ylabel('Alpha (%)', fontsize=11)
    ax.set_title(f'Alpha (Strategy − B&H) — Beat: {(alpha_vals>0).sum()}/{len(alpha_vals)}',
    ...
    ax = axes[1, 0]
    dd_vals = res_df['Max DD (%)'].values
    dd_colors = plt.cm.RdYlGn_r(np.abs(dd_vals) / max(np.abs(dd_vals).max(), 1.0))
    ax.bar(markets, dd_vals, color=dd_colors, edgecolor='white', width=0.55)
    for i, d in enumerate(dd_vals):
        ax.text(i, d - 1.0, f'{d:.2f}%', ha='center', va='top', fontsize=10, fontweight='bold')
    ...
    # Panel 4: Risk-Reward Scatter
    ax = axes[1, 1]
    for _, r in res_df.iterrows():
        ax.scatter(abs(r['Max DD (%)']), r['Return (%)'],
                   s=400, c=market_colors.get(r['Market'], '#888'),
                   edgecolors='black', linewidths=2, alpha=0.85, zorder=3)
        ax.annotate(r['Market'],
                    (abs(r['Max DD (%)']), r['Return (%)']),
                    xytext=(8, 0), textcoords='offset points',
                    fontsize=11, fontweight='bold', va='center')
        # B&H reference
        ax.scatter(abs(r['Max DD (%)']), r['B&H (%)'],
                   s=80, c=market_colors.get(r['Market'], '#888'),
                   marker='x', alpha=0.5, zorder=2)
    ax.axhline(0, color='gray', linestyle='--', alpha=0.5)
    ax.set_xlabel('|Max Drawdown| (%)', fontsize=11)
    ax.set_ylabel('Return (%)', fontsize=11)
    ax.set_title('Risk vs Return — circles = Strategy, X = B&H', fontsize=12, fontweight='bold')
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    filepath = os.path.join(save_dir, 'dashboard_summary.png')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--combined':
        main_combined()
    elif len(sys.argv) > 1 and sys.argv[1] == '--combined-v2':
        main_combined_v2()
    else:
        main()
